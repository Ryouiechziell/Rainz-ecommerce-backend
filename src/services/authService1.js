...
const { getUserByEmail, getUserByEmailAndPassword, createUser } = require("../models/userModel.js");
const { InternalServerError, NotFoundError, BadRequestError} = require("../utils/customErrors");
const { generateToken } = require("../utils/jwt.js");

async function authRegisterService({ username, password, email, role }) {
  const processStart = performance.now();
  const user_id = "USR-" + uuidv4();
  const hinter = "[AUTH REGISTER]"
  logger.debug(`${hinter} USER ID: ${user_id}, USERNAME: ${username}, PASSWORD: ${password}, EMAIL: ${email}, ROLE: ${role}`)
  ...
  try{
    ...
    [isInserted] = await createUser(user_id, username, email, hashedPassword, role);
    checkDbLatency(dbStart, 400, hinter);
...
async function authLoginService({email, password}) {
  const processStart = performance.now();
  const hinter = "[AUTH LOGIN]"
  logger.debug(`${hinter} EMAIL: ${email}, PASSWORD: ${password}`)
  ...
  if (!isMatch) {
    ...
    throw new BadRequetsError("Email atau password salah");
  }
...
async function authLoginGoogleService() {
  ...
  return url
    ...
    logger.error(`${hinter} RUNTIME ERROR ${err.stack}`);
    throw new InternalServerError("Terjadi kesalahan saat membuat redirect uri");
  }
}

async function authLoginGoogleCallbackService({code}) {
  const processStart = performance.now();
  ...
  try{
    ...
    logger.debug(`${hinter} TOKENS: ${tokens}`)
